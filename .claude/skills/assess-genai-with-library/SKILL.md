...
- **Harm / surface·source / weakness (in those steps).** `harm` = the consequence class; `surface`/`source` = which trust boundary the untrusted influence crosses and who drives it; `weaknesses` = the architectural conditions (cause + where + defect), i.e. how the system is exploitable. The match is **advisory, not a checklist**: does the weakness fit THIS architecture.
...
- **Risk reduction (in the mitigation step).** `mcp__keel__get_threat(id)` → linked mitigations; keep the ones applicable to the architecture. Mind each link's `strength`: a `soft` link only lowers likelihood (by the methodology's cross-cutting rule it is not an architectural control and does not finalize the threat); a `gating` link is an architectural control that blocks the threat.
