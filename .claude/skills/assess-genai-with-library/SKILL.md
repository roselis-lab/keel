...
## Overview
The library is the methodology **crystallized** into an impact-centric catalog (`impact_class` + `vulnerability` patterns + `reachability` + mitigations). It gives you **candidates and coverage**, but you must reason **FROM THE SYSTEM**: the catalog is a floor, not a ceiling, and not a substitute for analysis.
...
- **Threat search (in that methodology step).** Don't pull the whole catalog with all fields in one call — a full `list_threats(brief=false, include=[all])` doesn't fit in one tool response. Query pattern: **(1)** a light index `mcp__keel__list_threats(brief=false, include=["impact_class","vulnerability"])` for a first-pass pattern match; **(2)** `mcp__keel__get_threat(id)` per relevant candidate — for `reachability` and linked mitigations. This does NOT override the methodology's rule "search from the system, not from a catalog": the catalog is a floor. After matching, walk the 6 `impact_class` values and check whether an asset is under threat that the catalog misses — analyze such a threat from scratch by the methodology.
- **Asset / surface / vulnerability (in those steps).** `impact_class` = asset; `vulnerability` items = patterns of "how the system is exploitable". The match is **advisory, not a checklist**: does the pattern fit THIS architecture.
...
- **Risk reduction (in the mitigation step).** `mcp__keel__get_threat(id)` → linked mitigations; keep the ones applicable to the architecture. Mind `type`: `PREVENTIVE_SOFT` only lowers likelihood (by the methodology's cross-cutting rule it is not an architectural control and does not finalize the threat); `PREVENTIVE_HARD` blocks.
...
- **Catalog as a ceiling** — not walking `impact_class` coverage, missing a threat outside the catalog.
- **Copying the prose** of `vulnerability`/`reachability` instead of tying the wording to the specific system.
- **SOFT taken as a guarantee** — a `type=PREVENTIVE_SOFT` control does not close the threat.
